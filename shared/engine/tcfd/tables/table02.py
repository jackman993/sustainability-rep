...
# ================= 📝 Table 2: Physical Risks 生成邏輯 =================
def create_slide_2_physical(prs=None, output_filename=None, data_lines=None):
    ...
    set_text(cell, "Physical Risks", 16, True); set_cell_bg(cell, COLOR_BG_WHITE)
    ...
    # --- Row 2: Acute Risk (白色背景) ---
    # Type
    set_text(table.cell(2, 0), "Acute\nRisk", 9, True, COLOR_TEXT_SUB)
    
    # Factor (範例: 極端氣候)
    set_text(table.cell(2, 1), "Extreme weather\nevents", 9, False, COLOR_TEXT_SUB)
    ...
    # --- Row 3: Chronic Risk (灰色斑馬紋背景) ---
    # Type
    set_text(table.cell(3, 0), "Chronic\nRisk", 9, True, COLOR_TEXT_SUB)
    
    # Factor (範例: 溫度/海平面上升)
    set_text(table.cell(3, 1), "Changing weather\npatterns", 9, False, COLOR_TEXT_SUB)
    ...
    # 填充 LLM 返回的數據（如果有）
    if data_lines:
        data_rows = [2, 3]  # Row 2 (Acute), Row 3 (Chronic)
    ...
    if output_mode and output_filename:
        ...
        print(f"✅ Table 2 (Physical Risks) 生成完成: {output_filename}")

def generate_table_02(data_lines=None, filename=None, prs=None):
    # 如果提供了 prs，直接添加到主 prs；否則創建獨立文件（向後兼容）
    if prs is not None:
        create_slide_2_physical(prs=prs, data_lines=data_lines)
        return None  # 已添加到主 prs，不需要返回文件名
    else:
        if filename is None:
            filename = 'TCFD_table02_physical_risks.pptx'
        create_slide_2_physical(output_filename=filename, data_lines=data_lines)
        return filename

if __name__ == "__main__":
    create_slide_2_physical("tcfd_table_2_physical.pptx")
    try:
        from google.colab import files
        files.download('tcfd_table_2_physical.pptx')
    except:
        pass