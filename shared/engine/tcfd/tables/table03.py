...
def create_slide_3_resource_energy(prs=None, output_filename=None, data_lines=None):
...
def generate_table_03(data_lines=None, filename=None, prs=None):
    # 如果提供了 prs，直接添加到主 prs；否則創建獨立文件（向後兼容）
    if prs is not None:
        create_slide_3_resource_energy(prs=prs, data_lines=data_lines)
        return None  # 已添加到主 prs，不需要返回文件名
    else:
        if filename is None:
            filename = 'TCFD_table03_resource_energy.pptx'
        create_slide_3_resource_energy(output_filename=filename, data_lines=data_lines)
        return filename
