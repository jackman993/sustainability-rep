...
def create_slide_transformation_corrected(prs=None, output_filename=None, data_lines=None):
    ...
    if data_lines:
        ...
        # 表格結構：Row 2-3 的第 3-5 列需要填充（專注於 Policy & Regulation 和 Product & Tech）
        data_rows = [2, 3]  # 只填充兩行數據：Policy & Regulation 和 Product & Tech
        
        for idx, data_line in enumerate(data_lines[:2]):  # 只處理前2行數據
