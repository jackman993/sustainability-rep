...
# ================= 📝 Table 4: Products & Markets 生成邏輯 =================
def create_slide_4_products_markets(prs=None, output_filename=None, data_lines=None):
    ...
    set_text(cell, "Market Strategy", 16, True); set_cell_bg(cell, COLOR_BG_WHITE)
    ...
    # --- Row 2: Products & Services (白底) ---
    # Type
    set_text(table.cell(2, 0), "Products &\nServices", 9, True, COLOR_TEXT_SUB)
    
    # Factor (範例: 低碳商品開發)
    set_text(table.cell(2, 1), "Development of\nlow emission goods", 9, False, COLOR_TEXT_SUB)
    ...
    # --- Row 3: Markets & Resilience (灰底) ---
    # Type
    set_text(table.cell(3, 0), "Markets &\nResilience", 9, True, COLOR_TEXT_SUB)
    
    # Factor (範例: 進入新市場)
    set_text(table.cell(3, 1), "Access to new\nmarkets", 9, False, COLOR_TEXT_SUB)
    
    # Period: 長期 (依您指示)
    set_text(table.cell(3, 2), "Long-term", 9, False, COLOR_TEXT_SUB)
    ...
    if output_mode and output_filename:
        ...
        print(f"✅ Table 4 (Products & Markets) 生成完成: {output_filename}")

def generate_table_04(data_lines=None, filename=None, prs=None):
    # 如果提供了 prs，直接添加到主 prs；否則創建獨立文件（向後兼容）
    if prs is not None:
        create_slide_4_products_markets(prs=prs, data_lines=data_lines)
        return None  # 已添加到主 prs，不需要返回文件名
    else:
        if filename is None:
            filename = 'TCFD_table04_products_services.pptx'
        create_slide_4_products_markets(output_filename=filename, data_lines=data_lines)
        return filename

if __name__ == "__main__":
    create_slide_4_products_markets("tcfd_table_4_products_markets.pptx")
    try:
        from google.colab import files
        files.download('tcfd_table_4_products_markets.pptx')
    except:
        pass